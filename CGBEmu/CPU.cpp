...
void CPU::init() {
	...
	// Inicializaci�n de registros de I/O (Audio, Video, etc)
...
// Ejecuta UNA instrucci�n y devuelve los ciclos consumidos
// ---------------------------------------------------------
int CPU::step() {
	uint8_t cb_opcode;
	uint8_t opcode;
	...
	if (isHalted) {
		...
		// 2. LEER REGISTROS DE INTERRUPCI�N
		...
		// Si hay alguna interrupci�n pendiente (IF) que est� habilitada (IE)
		...
		// la CPU procesar� la interrupci�n normalmente.
		return cicles;
	}
	...
	case 0xCB:
		...
		// de CB manejen el incremento de PC o hazlo aqu� si es necesario.
	...
	default:
		...
		// Para debugging, puedes pausar aqu� o devolver 0.
		break;
	}
	...
	// Devuelve el total de ciclos que consumi� esta instrucci�n (calculado por addCycles interno)
	return cicles;
}
...
void CPU::HALT() {
	...
	// Caso 2: IME desactivado PERO hay interrupciones pendientes - HALT bug
		...
		// HALT bug: no incrementar PC, la siguiente instrucci�n se ejecuta dos veces
		isHalted = false;
		// NO incrementar pc aqu�
		addCycles(4);
	}
...
void CPU::DAA() {
	...
	// Caso A: La �ltima operaci�n fue una SUMA (N es falso)
	if (!flags.N) {
	...
	// Caso B: La �ltima operaci�n fue una RESTA (N es verdadero)
	else {
		if (flags.H) {
			adjustment |= 0x06;
		}
	...
	// Actualizar Flags
	flags.Z = (reg.A == 0);
	flags.H = false; // El flag H siempre se limpia despu�s de DAA
...
void CPU::ADD_SP_N() {
	...
	// ERROR ESTABA AQUI: No hagas cast a (uint8_t) aqu�. Usa todo el SP.
	int result = (uint16_t)mmu.sp + n;
