...
public:
	...
	void serviceInterrupt(int interrupt_id);
...
private:
	...
	void RLC_N(uint16_t opcode);

	//RL n
	/*Rotate n left. through Carry flag*/
	void RL_N(uint16_t opcode);

	//RRC n
	/*Rotate n right. Old bit 7 to Carry flag*/
	void RRC_N(uint16_t opcode);

	//RR n
	/*Rotate n right. through Carry flag*/
	void RR_N(uint16_t opcode);
	...
	void SRA_N(uint16_t opcode);

	//SRL n
	//Shift n right into Carry. MSB set to 0.
	void SRL_N(uint16_t opcode);
