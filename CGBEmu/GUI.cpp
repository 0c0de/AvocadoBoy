...
void drawMMU(MMU *mmu) {
	if (isActiveMemoryViewer) {
		...
		if (showGSA) {
			...
			ImGui::Text("Value on that pointer: %04x", mmu->read(mmu->sp));

			ImGui::Text("Value on actionButtons: %02x", mmu->actionButton & 0x0F);
			ImGui::Text("Value on directionButtons: %02x", mmu->directionsButton & 0x0F);
		}
